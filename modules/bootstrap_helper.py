...
import tqdm
import csv
from commons import show_image
...
class BootstrapHelper(object):
  ...
  def bootstrap(self, per_pose_class_limit=None):
    ...
    for pose_class_name in self._pose_class_names:
      ...
      csv_out_path = self._csvs_out_path
          ...
          if pose_landmarks is not None:
            ...
            for i, lmk in enumerate(pose_landmarks.landmark):
              if i not in [1,3,4,6,9,10,17,18,19,20,21,22,29,30,31,32]:
                pose_landmarks2.append(lmk.x * frame_width)
                pose_landmarks2.append(lmk.y * frame_height)
                pose_landmarks2.append(lmk.z * frame_width)
            pose_landmarks2 = np.array(pose_landmarks2, dtype=np.float32)
            ...
            csv_out_writer.writerow([image_name] + pose_landmarks2.flatten().astype(np.str).tolist())
